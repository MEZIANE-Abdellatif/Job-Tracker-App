...
describe("GET /profile/companies", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it("returns 401 without Authorization", async () => {
    const res = await request(app).get("/profile/companies").expect(401);
    expect(res.body).toEqual({ statusCode: 401, message: "Unauthorized" });
  });

  it("returns company insights for authenticated user", async () => {
    ...
    prismaMock.application.findFirst.mockResolvedValue({ company: "Initech" });
